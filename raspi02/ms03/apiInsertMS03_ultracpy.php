<?php
		...
		$isodate = new ISODate($date,$time);

/* 		$orig_date = new DateTime($date);
		$utcdatetime = new MongoDB\BSON\UTCDateTime($orig_date); */

        $insRec       = new MongoDB\Driver\BulkWrite;
        $doc = ['_id' => new MongoDB\BSON\ObjectID, 'MS' => $ms , 'ultra' => $ultra ,'date' => $isodate, 'time' => $time , 'status' => 1];
        $insRec->insert($doc);
